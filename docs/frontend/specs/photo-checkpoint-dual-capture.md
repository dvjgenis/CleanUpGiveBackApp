...
- [x] **AC-9:** **Session start** mode (`?mode=session-start`): form **Start Session** pushes `/photo-capture?mode=session-start`; submit consumes pending form → `startNewLiveSession` + first checkpoint → `/live-session`; cancel clears pending form and goes through `/hold-on` → Home (see AC-5)
...
- [x] **AC-5:** **Cancel** / Go Back uses `dismissTo('/live-session')` for in-session checkpoints / session-end; **session-start Cancel** clears pending setup, `CommonActions.reset` to `/hold-on` (progress ~1.6s), then Hold On fades out and Home fades in (`enter=fade` + `requestHomeFadeIn`)
