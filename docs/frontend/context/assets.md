...
| `frontend/assets/animations/` | Motion exports — `photo-submitted.lottie` (Camera Pop-Up, repeater-baked for RN; Metro `assetExts` includes `lottie`), `missed-checkpoint.json`, `hourglass.json` (free-trial paywall), `shipping.json` / `shipping.svg` (order-email truck; GIF is the email-safe render), plus alt JSON/GIF sources |
