...
| `frontend/assets/figma/shop/` | Shop home product PNGs; flow subfolders `cart/`, `checkout/`, `confirmation/`, `donate/`, `product-detail/` (tote carousel: `tote-bags-photo-flatlay.png` + `tote-bags-photo.png` + `tote-{green,blue}-{planet-b,earth-friendly}.png`; kit/vest flatlays: `kit-hero-flatlay.png`, `adult-vest-flatlay.png`, `child-vest-flatlay.png`); ported glyph originals in `_source/` |
...
| `frontend/assets/animations/` | Motion exports — `photo-submitted.lottie` (Camera Pop-Up, repeater-baked for RN; Metro `assetExts` includes `lottie`), `missed-checkpoint.json`, `hourglass.json` (free-trial paywall), `broom.svg` (BrandLoadingView broom — SVG Repo emoji broom, dirt dots removed, flipped so bristles face right; CUGB fills: `amber/700` handle, `lime/500` bristles, `gray/700` collar, `gray/900` stroke), `cloud.svg` (SVG Repo [basic-cloud](https://www.svgrepo.com/svg/21417/basic-cloud); fill `tour/mint` `#dcebe2`), `tidy-man-preview.html` (full-screen mobile preview: cream sky with mint clouds wrapping right at 16 px/s, sizes vary, full-width green ground from 55%, cream Sanchez label in the green), unused `tidy-man.json` leftover, `shipping.json` / `shipping.svg` (order-email truck; transparent GIF is the email-safe render), plus alt JSON/GIF sources |
