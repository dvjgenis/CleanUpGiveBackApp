...
| HomeScreen | `screens/HomeScreen.tsx` | Home dashboard (`home_dashboard___final_branding`, Figma `406:291`); Service Hours / streak / Your Impact from `sessionStatsStore` + `homeDashboardStats`; chart Y-axis uses integer-only ticks (`niceMax` → multiple of 4); greeting uses `getTimeOfDayGreeting` + preferred name from `onboardingStore`; week picker drives real chart buckets; recent sessions from `recentSessionsStore`; recent events → `/event-detail` |
