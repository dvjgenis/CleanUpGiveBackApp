...
| FreeTrialModal | `features/session-tracking/components/FreeTrialModal.tsx` | Paywall modal (Figma `1141:2178`) — appears when `elapsedSeconds >= 3600` on the live tracker (unless `trackerPaymentStore.hasPaid`); opaque `bgApp` full-screen backdrop (no dim/vignette, matches Figma), centered card with green border, `HourglassIcon` Lottie, $49.99 one-time pricing; Continue → `/checkout?mode=tracker&returnTo=live-session`; Pay Later → dismisses |
...
| FeedbackThankYouScreen | `screens/FeedbackThankYouScreen.tsx` | Hand-designed (no Figma source) — sits between `FeedbackScreen`'s Submit and `SubmissionConfirmationScreen`; reuses the `FeedbackScreen` centered-card shell + a `CheckCircleIcon` spring pop-in; **Continue** → `/submission-confirmation`. Reached only via Submit; Skip bypasses it. |
...
| AccountScreen | `screens/AccountScreen.tsx` | Account tab (Figma `569:896`); profile hero, Records/Shop/Preferences/Permissions; Camera/Location/Notifications toggles wired to OS permission request + Settings fallback; Profile nav active |
