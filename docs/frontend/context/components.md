...
| MapTypesSheet | `features/session-tracking/components/MapTypesSheet.tsx` | Live-tracker Map Types bottom sheet (Standard / Satellite / Hybrid) — selection wired to `liveSessionStore.mapLayer` via `setLiveSessionMapLayer`, so it drives the MapLibre basemap live; brand-primary highlight on selected option |
