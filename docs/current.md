...
**API:** `EXPO_PUBLIC_API_URL=https://cleanup-sessions.fly.dev` in `frontend/.env`. Backend needs Fly secrets (`DATABASE_URL`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `ADMIN_API_KEY`); local Prisma uses `backend/sessions/.env` (session pooler `DATABASE_URL`). Redeploy Fly after service-letter PDF changes.
