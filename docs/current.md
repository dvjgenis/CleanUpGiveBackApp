...
- **Sessions list** refetches from Fly API on focus when `EXPO_PUBLIC_API_URL` is set — loading/empty/error states; when API is unset, shows filtered mock rows; **Select** → bulk delete non-approved sessions and **Download PDF** for selected approved sessions (AsyncStorage tombstones keep deleted ids hidden across restarts)
