...
- ✅ Insights (`/insights`, also `/analytics`) matches admin Insights layout: trend, queue age, decisions, court progress (View more when >5), donuts, US heatmap — trend/queue/decisions/donuts/heatmap/court-progress driven by live `sessions` + `court_orders` (empty tables → empty charts, no fixture fallback)
