...
- **Emails** (`/emails`, 2026-08-09): **Compose** shows a read-only **From** line (`EMAIL_FROM` / default `noreply@cleanupgiveback.org`), unified To (volunteer search or typed email — no Volunteer/Custom tabs), Cc/Bcc, font/size/color in the rich-text toolbar, Send or Schedule send. **Scheduled** tab lists queued mail with Edit / Send now / Cancel for `pending` rows. **Templates** uses everyday-language insert chips (e.g. “Volunteer name”) — Donna never types `{{brackets}}`. Apply `admin/db/013_scheduled_emails.sql`; set `CRON_SECRET` for `/api/cron/send-scheduled-emails` (`vercel.json` every minute)
