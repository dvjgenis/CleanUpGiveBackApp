...
**Order emails (2026-08-12):** Figma `1311:359` placed + shipped share `lib/order-email-html.ts` (keep in sync with `backend/sessions/src/lib/order-email-html.ts`). Shipped send: `updateOrderFulfillment` → `sendShopOrderEmail`. Placed send: Fly `POST /emails/order-placed`. Hosted `/email/*` on Vercel (transparent shipping GIF 220×106, Sanchez headline 28px, Noto body/labels/footer; greeting is one wrapping HTML line). Body fill is cream `#fcf9f8`. `letter-spacing: 0.02em` matches Forgot Password and hours-reminder. Do not CID-inline. Not editable in Templates. Spec: [backend/specs/order-emails.md](backend/specs/order-emails.md). Apply [`admin/db/020_order_emails_figma.sql`](../admin/db/020_order_emails_figma.sql).
