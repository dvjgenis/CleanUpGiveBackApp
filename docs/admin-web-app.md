...
- **Court Risk** (`/court-risk`, 2026-08-07): At-risk court-ordered volunteers — deadline, hours progress, 30-day invalid-session count, near-deadline session-volume spike
...
- **Emails** (`/emails`, 2026-08-07): **Compose** tab sends a real email straight from the dashboard — pick a volunteer or a custom address, rich-text body (WYSIWYG, not raw HTML), inline images + file attachments, optionally start from a template. **Templates** tab manages the 5 system (automated-send) templates plus any custom templates Donna creates for reuse in Compose
...
| `/emails` (2026-08-07) | `email_templates` table (`admin/db/011_email_templates.sql` — id-keyed, `is_system` flag, missing system row falls back to hardcoded `DEFAULT_TEMPLATES`) + Auth directory for the recipient picker | `listAllTemplates`/`getTemplate` (`lib/email-templates.ts`), `getVolunteerDirectory`; writes via `updateEmailTemplate`/`createTemplate`/`updateTemplate`/`deleteTemplate` (`actions/emailTemplates.ts`) and `sendAdHocEmail` (`actions/emails.ts`) |
