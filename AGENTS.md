...
- Mobile shop selected chips and cart accents use tertiary brand color, not gray or lime. Home **Your Impact** favors a visual Recent Cleanups feed with a concise period summary (not a dense stats grid); month/year pickers are list-only with no type-to-search.
...
- Shipping for Donna: Phase 1 Pirate Ship labels + paste tracking in Orders UI + Resend email; Phase 2 **Shippo** (preferred over EasyPost) for checkout rates, auto label buy via Supabase Edge Functions after Stripe payment, and webhook tracking — see `docs/reports/2026-08-20-shippo-vs-easypost-donna.md` and `docs/research/shipping-integration-2026-08.md` (Shippo not connected yet; API keys server-side only). Checkout offers pickup/local vs shipped; app charges for access, not kit cost. Shop USPS shipping is 25% of paid product subtotal (free cleanup kit ignored). Free cleanup kit confirmation emails should show free shipping and a $59.99 total.
