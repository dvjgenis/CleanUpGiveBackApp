...
const TRACKER_ACCESS_ITEM: CartLineItem = {
  ...
  description: 'Unlimited tracking with cleanup kit',
...
export function CheckoutScreen() {
  ...
  const includesKit = isTrackerMode || cartHasKit;
  const shippingLabel =
    fulfillmentMethod === 'usps_ship' ? summary.shippingLabel : 'Office pickup';
  const trackerLineHint =
    fulfillmentMethod === 'usps_ship'
      ? 'App access + cleanup kit — free shipping'
      : 'App access + cleanup kit — office pickup';
  ...
  async function handlePlaceOrder() {
    ...
    try {
      const orderItems = isTrackerMode
        ? [TRACKER_ACCESS_ITEM, TRACKER_KIT_ITEM]
        : items;
      ...
      const orderResult = await createShopOrder({
        ...
        shippingFee: isTrackerMode ? 0 : computeShippingFee(items, fulfillmentMethod),
        fulfillmentMethod,
        includesKit,
      });
      ...
      <View style={s.body}>
        ...
        >
          ...
          <View style={s.card}>
            ...
            {isTrackerMode ? (
              <Text style={s.kitToggleLabel}>A cleanup kit is included with tracking access.</Text>
            ) : null}
          </View>
